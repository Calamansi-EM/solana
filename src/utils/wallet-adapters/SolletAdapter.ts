...
import { PublicKey, Transaction, clusterApiUrl, Connection, TransactionSignature, Signer } from "@solana/web3.js";
...
const BASE_URL = "https://www.sollet.io";

export class SolletAdapter extends Wallet implements WalletAdapter {
...
const createSolletWallet = (env: Environment, connection: Connection): SolletAdapter => {
  const network = clusterApiUrl(env);
  const wallet = new SolletAdapter(BASE_URL, network, connection);
  return wallet;
};

export default createSolletWallet;
