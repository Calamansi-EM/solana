...
export type DirectionType = { buy: any } | { sell: any };

export const DirectionEnum = rustEnum([struct([], "buy"), struct([], "sell")]);

export const Direction: Layout<DirectionType> = rustEnum([struct([], "buy"), struct([], "sell")]);

export const DIRECTION_SPAN = Math.max(...Object.values(DirectionEnum.registry).map((r) => r.span));

export function encodeDirection(d: DirectionType) {
...
export type OrderTypeType = { auctionOrder: any } | { auctionLimitOrder: { limit: BN } };

export const OrderTypeEnum = rustEnum([struct([], "auctionOrder"), struct([u64("limit")], "auctionLimitOrder")]);

export const OrderType: Layout<OrderTypeType> = rustEnum([
  struct([], "auctionOrder"),
...
export function encodeOT(d: OrderTypeType) {
...
export type HoldingDataInitType = {
...
export const HoldingDataInit: Layout<HoldingDataInitType> = struct([
...
export function printHoldingDataInit(h: HoldingDataInitType) {
...
export type LimitPriceMintType = {
  limit_price: BN;
...
export const LimitPriceMint: Layout<LimitPriceMintType> = struct([
  u128("limit_price"),
...
export function printLimitPriceMint(h: LimitPriceMintType) {
...
export type LimitPriceType = {
  limit_price: BN | null;
};

export const LimitPrice: Layout<LimitPriceType> = struct([option(u128("limit_price"), "limit_price")]);
...
export type HoldingDataType = {
  ...
  limit_prices_and_mint: LimitPriceMintType[];
};

export const HoldingData: Layout<HoldingDataType> = struct([
  bool("is_initialized"),
  ...
  vec<LimitPriceMintType>(LimitPriceMint.replicate("limit_prices_and_mint"), "limit_prices_and_mint"),
]);
...
export async function printHolding(h: HoldingDataType, connection: Connection) {
...
export type PlaceOrderPayloadType = {
  ...
  amount: BN;
  limit_price: LimitPriceType;
};

export const PlaceOrderPayload: Layout<PlaceOrderPayloadType> = struct([
